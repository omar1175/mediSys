...
def _validate_availability(doctor, scheduled_at, duration_minutes=30):
    ...
    appt_end = scheduled_at + timezone.timedelta(minutes=duration_minutes)
...
class AppointmentCreateSerializer(serializers.ModelSerializer):
    class Meta:
        model = Appointment
        fields = ["doctor", "specialty", "scheduled_at", "duration_minutes", "notes"]
    ...
    def validate(self, attrs):
        ...
        if scheduled_at and scheduled_at < timezone.now():
            raise serializers.ValidationError({"scheduled_at": "Cannot book an appointment in the past."})

        if doctor and scheduled_at:
            _validate_no_overlap(doctor, scheduled_at, duration)

        if doctor and scheduled_at:
            _validate_availability(doctor, scheduled_at, duration)
