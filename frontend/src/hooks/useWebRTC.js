...
export function useWebRTC(appointmentId) {
  ...
  const handleOffer = useCallback(async (sdp) => {
    ...
    if (pc.signalingState !== "stable") {
      console.warn("[WebRTC] Received offer in state:", pc.signalingState, "- recreating PC");
      try { pc.close(); } catch {}
  ...
  handleSignalingRef.current = (data) => {
    ...
    switch (data.type) {
      ...
      case "call.both_connected": {
        setConnectionState(CONNECTION_STATES.CONNECTING);
        if (isInitiator.current && pcRef.current?.signalingState === "stable") {
          console.log("[WebRTC] Both connected, initiator retrying offer...");
          hasCreatedOffer.current = false;
          createOffer();
        }
        break;
